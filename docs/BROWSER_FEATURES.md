...
| Passkeys | Partial | Requests macOS browser passkey access through AuthenticationServices when the app is signed with Apple's browser public-key-credential entitlement. WebKit handles WebAuthentication prompts once macOS authorizes the browser. Local builds ad-hoc sign with the entitlement by default and expose a Settings toggle to suppress public-key credential calls only when a site gets stuck in nearby-device Bluetooth fallback. | `BrowserAppDelegate.m`, `TrailBrowser.entitlements`, `Makefile`, `Settings.*` |
