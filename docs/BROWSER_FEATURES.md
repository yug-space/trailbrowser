...
| Tabs | Done | Sidebar tabs, pin/unpin, drag reordering, close/new, favicon/title, live tab switching, memory-pressure sleeping, `Cmd+Opt+Tab` switcher, `Cmd+Shift+T` closed-tab restore, and tab context actions for reload, duplicate, move, close others, and close tabs to the right. | `BrowserAppDelegate.m`, `BrowserTabViews.*` |
