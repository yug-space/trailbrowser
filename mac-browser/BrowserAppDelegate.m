...
- (NSString *)disabledPublicKeyCredentialScript {
    return
        @"(() => {"
         ...
         "const publicKeyRequest = (options) => options && typeof options === 'object' && Object.prototype.hasOwnProperty.call(options, 'publicKey');"
         "const install = (target, name, fn) => {"
         "if (!target) return false;"
         "try { Object.defineProperty(target, name, { configurable: true, writable: true, value: fn }); return true; } catch (_) {}"
         "try { target[name] = fn; return target[name] === fn; } catch (_) {}"
         "return false;"
         "};"
         ...
         "const guardedGet = (options) => publicKeyRequest(options) ? Promise.reject(unavailable()) : originalGet(options);"
         "const guardedCreate = (options) => publicKeyRequest(options) ? Promise.reject(unavailable()) : originalCreate(options);"
         "if (originalGet) { install(prototype, 'get', guardedGet); install(credentials, 'get', guardedGet); }"
         "if (originalCreate) { install(prototype, 'create', guardedCreate); install(credentials, 'create', guardedCreate); }"
         "}"
         "const no = () => Promise.resolve(false);"
         "if (window.PublicKeyCredential) {"
         "install(window.PublicKeyCredential, 'isUserVerifyingPlatformAuthenticatorAvailable', no);"
         "install(window.PublicKeyCredential, 'isConditionalMediationAvailable', no);"
         "install(window.PublicKeyCredential, 'isExternalCTAP2SecurityKeySupported', no);"
         "}"
         "try { Object.defineProperty(window, 'PublicKeyCredential', { configurable: true, writable: true, value: undefined }); } catch (_) {}"
         "})()";
}
